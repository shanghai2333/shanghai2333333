...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      if( _week != 4 && _week != 5 && _week != 6 ){
        $('.calendar-today .triangle').css('left','-16px');
        $('.calendar-today .triangle').css('border-color','transparent rgb(255, 128, 142) transparent transparent');
        offset_t = $(obj).offset().top - $(window).scrollTop() + (-this.$calendar_today.height() + $(obj).height()) / 2;    
        offset_l = $(obj).offset().left + $(obj).width();
        this.$calendar_today.show();
        this.$calendar_today
            .css({left: offset_l + 30 , top: offset_t, opacity: 1})
            .stop()
            .animate({left: offset_l + 16, top: offset_t, opacity: 1});
      }
      else{
        $('.calendar-today .triangle').css('left','119px');
        $('.calendar-today .triangle').css('border-color','transparent transparent transparent rgb(255, 128, 142)');
        offset_t = $(obj).offset().top - $(window).scrollTop() + (-this.$calendar_today.height() + $(obj).height()) / 2;
        offset_l = $(obj).offset().left - this.$calendar_today.width() - 16;
    ...
    renderDOM: function () { // 渲染DOM
      ...
      var _titleStr = '<a href="#" class="title"></a>'+
                      '<a href="javascript:;" id="backToday">'+
                        '<span class="icon-today"></span>'+
                      '</a>'+
                      '<a href="javascript:;" id="addEvent">'+
